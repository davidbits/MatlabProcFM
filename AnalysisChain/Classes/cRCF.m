...
classdef cRCF < matlab.mixin.Copyable
    ...
    methods
        ...
        function oRCF = readSamplesFromFile(oRCF, StartSample, NSamples)
            ...
            fprintf('Reading %d samples starting from sample %d, ending at %d\n', NSamples, StartSample, NSamples+StartSample-1);
            maxSamplesAvailable = oRCF.m_NSamples;
            fprintf('Maximum samples available: %d\n', maxSamplesAvailable);

            %Read NSamples
            for ii = 1:NSamples
                % every 100000 samples print percentage read
                if mod(ii, 100000) == 0
                    fprintf('%.2f%% complete...\n', (ii / NSamples) * 100);
                end
