...
% Purpose: Rigorous FERS multi-transmitter bug detection using physically-
%          scaled complex IQ data via loadfersHDF5.
%
% Improvements over previous version:
%   - Uses loadfersHDF5() to apply the fullscale attribute (ADC -> physical units)
%   - Compares full complex IQ signals, not just I-channel
%   - Covers all three scenarios (Clean, Wideband Jammer, Single-Tone Jammer)
%   - Covers both channels (Reference and Surveillance)
%   - Reports fullscale factors to detect normalisation differences
%   - Inspects Q-channel power to evaluate Q=0 hypothesis
...
files.clean.ref = 'Input/CleanSingleTarget/ArmasuisseRefRxClean.h5';
files.clean.sur = 'Input/CleanSingleTarget/ArmasuisseSurRxClean.h5';

% Wideband FM Jammer (FM transmitter + 1W wideband jammer)
files.wbjam.ref = 'Input/JamSingleTarget/ArmasuisseRefRxJam.h5';
files.wbjam.sur = 'Input/JamSingleTarget/ArmasuisseSurRxJam.h5';

% Single-Tone Jammer (FM transmitter + 1W CW tone jammer)
files.tonejam.ref = 'Input/JamSingleTarget_tone/ArmasuisseRefRxToneJam.h5';
files.tonejam.sur = 'Input/JamSingleTarget_tone/ArmasuisseSurRxToneJam.h5';
...
fprintf('Loading Single-Tone Jammer scenario...\n');
[tonejam_ref, tonejam_ref_meta] = load_complex(files.tonejam.ref);
[tonejam_sur, tonejam_sur_meta] = load_complex(files.tonejam.sur);
...
fprintf('  Tone Jam Ref: %.6e   Sur: %.6e\n', tonejam_ref_meta.scale, tonejam_sur_meta.scale);
...
if abs(clean_ref_meta.scale - tonejam_ref_meta.scale) / clean_ref_meta.scale > 0.01
    fprintf('  [NOTE] Ref fullscale differs between Clean and Tone Jammer by %.1f%%\n', ...
        100 * abs(clean_ref_meta.scale - tonejam_ref_meta.scale) / clean_ref_meta.scale);
end
...
len_ref_tone = min(length(clean_ref), length(tonejam_ref));
len_sur_tone = min(length(clean_sur), length(tonejam_sur));
...
fprintf('  %-30s  %14.6e  %14.6e\n', 'Tone Jammer (FM+1W)',     pwr_tonejam_ref, pwr_tonejam_sur);
fprintf('\n');

delta_pwr_ref_wb   = pwr_wbjam_ref   - pwr_clean_ref;
delta_pwr_sur_wb   = pwr_wbjam_sur   - pwr_clean_sur;
delta_pwr_ref_tone = pwr_tonejam_ref - pwr_clean_ref;
delta_pwr_sur_tone = pwr_tonejam_sur - pwr_clean_sur;

fprintf('  Delta Power (Jammer - Clean):\n');
fprintf('  %-30s  %+14.6e  %+14.6e\n', 'Wideband Jammer', delta_pwr_ref_wb,   delta_pwr_sur_wb);
fprintf('  %-30s  %+14.6e  %+14.6e\n', 'Tone Jammer',     delta_pwr_ref_tone, delta_pwr_sur_tone);
fprintf('\n');
...
fprintf('  %-30s  %+13.2f%%  %+14.2f%%\n', 'Tone Jammer', ...
    100 * delta_pwr_ref_tone / pwr_clean_ref, 100 * delta_pwr_sur_tone / pwr_clean_sur);
...
if delta_pwr_ref_tone < 0
    fprintf('  [FAIL] Ref power DECREASED by %.2f%% with tone jammer (violates P_total = P1 + P2)\n', ...
        abs(100 * delta_pwr_ref_tone / pwr_clean_ref));
else
    fprintf('  [PASS] Ref power increased or unchanged with tone jammer\n');
end
if delta_pwr_sur_tone < 0
    fprintf('  [FAIL] Sur power DECREASED by %.2f%% with tone jammer (violates P_total = P1 + P2)\n', ...
        abs(100 * delta_pwr_sur_tone / pwr_clean_sur));
else
    fprintf('  [PASS] Sur power increased or unchanged with tone jammer\n');
end
...
% --- Tone Jammer vs Clean ---
rho_ref_tone_complex = complex_corr(clean_ref(1:len_ref_tone), tonejam_ref(1:len_ref_tone));
rho_sur_tone_complex = complex_corr(clean_sur(1:len_sur_tone), tonejam_sur(1:len_sur_tone));
rho_ref_tone_real    = real_corr(clean_ref(1:len_ref_tone), tonejam_ref(1:len_ref_tone));
rho_sur_tone_real    = real_corr(clean_sur(1:len_sur_tone), tonejam_sur(1:len_sur_tone));
...
fprintf('  %-35s  %+10.6f  %+10.6f\n', 'Tone Jam vs Clean (complex |rho|)', rho_ref_tone_complex, rho_sur_tone_complex);
fprintf('  %-35s  %+10.6f  %+10.6f\n', 'Tone Jam vs Clean (real-part rho)', rho_ref_tone_real,    rho_sur_tone_real);

fprintf('\n  Decorrelation Assessment:\n');
rho_threshold = 0.99;
test_cases = { ...
    'Ref (WB Jammer, complex)',   rho_ref_wb_complex; ...
    'Ref (Tone Jammer, complex)', rho_ref_tone_complex; ...
    'Sur (WB Jammer, complex)',   rho_sur_wb_complex; ...
    'Sur (Tone Jammer, complex)', rho_sur_tone_complex; ...
};
...
channels = { ...
    ...
    'Tone Jam Ref', tonejam_ref; ...
    'Tone Jam Sur', tonejam_sur; ...
};
...
n_power_fail = (delta_pwr_ref_wb < 0) + (delta_pwr_sur_wb < 0) + ...
               (delta_pwr_ref_tone < 0) + (delta_pwr_sur_tone < 0);
n_corr_fail  = (rho_ref_wb_complex < rho_threshold) + (rho_ref_tone_complex < rho_threshold);

if n_corr_fail > 0 && n_power_fail > 0
    fprintf('  [!!!] BUG CONFIRMED\n');
    fprintf('  Both decorrelation (rho << 1) and superposition violation\n');
    fprintf('  (power decrease upon adding a source) are present after\n');
    fprintf('  fullscale correction. The FERS simulator is NOT correctly\n');
    fprintf('  summing multi-transmitter signals.\n');
elseif n_corr_fail > 0
    fprintf('  [!!!] BUG LIKELY\n');
    fprintf('  Reference channel decorrelation persists after fullscale\n');
    fprintf('  correction. Power anomaly may have been a scaling artefact,\n');
    fprintf('  but the signal content itself is corrupted.\n');
elseif n_power_fail > 0
    fprintf('  [WARN] Power anomaly persists but correlations are acceptable.\n');
    fprintf('  This may indicate a gain/scaling issue rather than signal overwriting.\n');
else
    fprintf('  [PASS] All checks passed. Signals appear correctly summed.\n');
end
